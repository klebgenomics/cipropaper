...
# logistic regression model summary - facet plot
```{r}
...
model_summary_tab_long %>% ggplot(aes(x=(as.numeric(value)), y=fct_rev(Label)))+
  ...
  geom_text_repel(aes(label=paste0(as.numeric(value)), colour=Prediction), size=3,position=pd)+
 ...
 facet_wrap(~ `Evaluation metric`, ncol = 6, scales="free_x")+
  ...
  facetted_pos_scales(
    x = list(`Evaluation metric` == "Categorical agreement" ~ scale_x_continuous(breaks = c(80,100), limits = c(70, 100)),
             `Evaluation metric` == "Sensitivity" ~ scale_x_continuous(breaks = c(80,100), limits = c(70, 100)),
             `Evaluation metric` == "Specificity" ~ scale_x_continuous(breaks = c(80, 100), limits = c(70, 100)),
             `Evaluation metric` == "Major error" ~ scale_x_continuous(breaks = c(0,10), limits = c(0,10)),
             `Evaluation metric` == "Very major error"~ scale_x_continuous(breaks = c(10,20), limits = c(0, 30)),
             `Evaluation metric` == "AIC"~ scale_x_continuous()
  ))
...
```
# rules-based classifier species summary - facet plot
```{r}
species_summary_table <- 
bind_rows(c("out"="R","species"="Klebsiella pneumoniae species complex", 
            metrics(cipro_antibiogram$predR, cipro_antibiogram$resistant)$summary), 
...
species_summary_table_long %>% ggplot(aes(x=(as.numeric(value)), y=fct_rev(Label)))+
  geom_point(aes(colour=Prediction), position=pd) +
  geom_text_repel(aes(label=as.numeric(value), colour=Prediction), size=3,position=pd)+
  scale_color_manual(values=c("R"="IndianRed", "NWT"="#78638a")) +
...
# NOT ALGINED facet plot - combined LR models + rules-based classifier
```{r}
...
# ALGINED facet plot - combined LR models + rules-based classifier
```{r}
...
ggsave(height=8, width=12, file="figs/LRmodel_classifier_performance_aligned.png")
ggsave(height=8, width=12, file="figs/LRmodel_classifier_performance_aligned.pdf")
```
