...
ST_R <- cipro_antibiogram %>% group_by(ST) %>% 
  ...
  mutate(out=rep("R", 1967)) %>% 
  mutate(category=rep("ST", 1967)) %>% 
...
Country_R <- cipro_antibiogram %>% group_by(Country) %>% 
  summarise(sens=sens(predR, resistant)*100, 
            ...
            percent_R=(sum(resistant))/n*100) %>%
  mutate(out=rep("R", 28)) %>% 
  mutate(category=rep("Country", 28)) %>%   
...
Combined_R_100_plot <-ggplot(Combined_R_100_long, aes(as.numeric(value),fct_inorder(as.factor(Category_label)), colour=`Evaluation metric`)) +
  geom_point(aes(shape = `Evaluation metric`), alpha=0.5, size=4) +
  scale_colour_manual(values=c(Sensitivity="#c698ea", Specificity="#9a98ea", `Resistant`="IndianRed")) +
  scale_y_discrete(limits=rev)+
  facet_grid(Category~., scales = "free", space='free')+ 
  labs(y="", x = "%", colour="", shape="") + 
  theme_bw()
