...
## plot logistic regression coefficients for each determinant
``` {r}
...
``` {r}
t1 <- read_csv("tables/Table1_freq_per_det_byPheno_summary_singleDet.csv")

ppvR <- t1 %>% mutate(p=R/N, se=sqrt(p*(1-p)/N), ci.lower=p-1.96*se, ci.upper=p+1.96*se) %>% select(Determinant, p, ci.lower, ci.upper) %>% mutate(category="R")

ppvNWT <- t1 %>% mutate(p=NWT/N, se=sqrt(p*(1-p)/N), ci.lower=p-1.96*se, ci.upper=p+1.96*se) %>% select(Determinant, p, ci.lower, ci.upper, N) %>% mutate(category="NWT")

geom_linerange(aes(xmin=ci.lower, xmax=ci.upper, group=model, col=model, linetype=interaction), position=pd) +
    geom_point(aes(x=est, group=model, col=model), position=pd) + 
    
ppv_plot <- ppvR %>% bind_rows(ppvNWT) %>%
  ggplot(aes(y=Determinant)) +
  geom_vline(xintercept=50, linetype=2) +
  geom_linerange(aes(xmin=ci.lower*100, xmax=ci.upper*100, group=category, col=category), position=pd) +
  geom_point(aes(x=p*100, group=category, col=category), position=pd) + 
  scale_colour_manual(values=c(NWT="#2c15ae", R="#bd1515")) +
  theme_bw() + labs(x="PPV (%)")
...
ppvNWT_plot <- ppvNWT %>% 
  ...
  scale_y_discrete(labels=paste0("n=",ppvNWT$N), position="right")
```
...
ggsave(width=8, height=9, file="figs/FigX_regressionModels.pdf")
ggsave(width=8, height=9, file="figs/FigX_regressionModels.png")
...
ppv_plot / ( log_reg_R_plot + ggtitle("R model") + log_reg_NWT_plot + ggtitle("NWT model") + plot_layout(guides="collect", axes="collect")) / (lin_reg_R_plot + ggtitle("MIC model") + lin_reg_DD_plot + ggtitle("Disk diffusion zone model") + plot_layout(axes="collect", guides="collect"))
