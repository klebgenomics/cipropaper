...
me <- function(pred, truth) {
  xtabs <- table(factor(pred, levels = c("S", "I", "R")),
                 factor(truth, levels = c("S", "I", "R")))
  
  major_n <- xtabs["R", "S"]   # predicted R, true S
  
  susceptible_n <- xtabs["S", "S"] + xtabs["R", "S"]   # all reference S isolates
...
vme <- function(pred, truth) {
  ...
  very_major_n <- xtabs["S", "R"]   # predicted S, true R
  
  resistant_n <- xtabs["S", "R"] + xtabs["R", "R"]  # all reference R isolates
  
  very_major_n / resistant_n
}

minor_error <- function(pred, truth) {
  xtabs <- table(factor(pred, levels = c("S","I","R")),
                 factor(truth, levels = c("S","I","R")))
...
write_tsv(multiVariable_vs_singleVariable, file="tables/TableS8_LogRegCoef.tsv")
```
...
count_model_summary <- logreg_details(model_R_groups) %>% mutate(Model="2a", response="R") %>%
  ...
  write_tsv(file="tables/TableS9_CountLogRegCoef.tsv")
...
model_summary_tab %>%
  ...
  write_tsv("tables/Table1a_ModelSummary.tsv", na="-")
...
species_R <- cipro_antibiogram %>% group_by(species) %>% 
  ...
  summarise(cat=cat_agree(predR, resistant), 
            ...
            me=me(predSIR, Resistance.phenotype), 
            vme=vme(predSIR, Resistance.phenotype),
            minor=minor_error(predSIR, Resistance.phenotype),
            n=n())%>%
  mutate(out=rep("R", 4)) %>% 
...
kpsc_R <- cipro_antibiogram %>% 
  summarise(cat=cat_agree(predR, resistant), 
            ...
            n=n())%>%
  mutate(out=rep("R", 1))%>%
  mutate(species=rep("Klebsiella pneumoniae species complex", 1))

kpsc_NWT <-  cipro_antibiogram %>% 
  summarise(cat=cat_agree(predNWT, nonWT_binary), 
            sens=sens(predNWT, nonWT_binary), 
            spec=spec(predNWT, nonWT_binary), 
            me=me(predSIR, Resistance.phenotype), 
            ...
            minor=minor_error(predSIR, Resistance.phenotype),
            n=n()) %>%
  mutate(out=rep("NWT", 1))%>%
  mutate(species=rep("Klebsiella pneumoniae species complex", 1))



species_summary_table <- 
bind_rows(kpsc_R,kpsc_NWT)%>%
  mutate_at(c("cat","sens","spec","me","vme","minor", "n"), as.double) %>%
  bind_rows(species_R) %>%
  bind_rows(species_NWT) %>%
  rename(Sensitivity=sens, Specificity=spec, ME=me, VME=vme, minor=minor, `Categorical agreement`=cat, N=n) %>%
  mutate(Sensitivity=paste0(round(as.numeric(Sensitivity)*100,2),"%")) %>%
  mutate(Specificity=paste0(round(as.numeric(Specificity)*100,2),"%")) %>%
  mutate(ME=paste0(round(as.numeric(ME)*100,2),"%")) %>%
  mutate(VME=paste0(round(as.numeric(VME)*100,2),"%")) %>%
  mutate(MinE=paste0(round(as.numeric(minor)*100,2),"%")) %>%
  mutate(`Categorical agreement`=paste0(round(as.numeric(`Categorical agreement`)*100,2),"%")) %>%
  select(out, species, `Categorical agreement`, Sensitivity, Specificity, ME, VME, MinE, N)
  

write_csv(species_summary_table, "tables/Table1b_ModelSummary.csv", na="-")
write_tsv(species_summary_table, "tables/Table1b_ModelSummary.tsv", na="-")
```
...
ST_R <- cipro_antibiogram %>% group_by(ST) %>% 
  summarise(sens=sens(predR, resistant)*100, 
            ...
            me=me(predSIR, Resistance.phenotype)*100, 
            vme=vme(predSIR, Resistance.phenotype)*100, 
            minor=minor_error(predSIR, Resistance.phenotype),
            n=n(),
...
Combined_R <- Combined_R %>%
  rename(Sensitivity=sens, Specificity=spec, ME=me, VME=vme, `Minor error`=minor, `Categorical agreement`=cat, N=n, `Resistant`=percent_R, Category=category, Prediction=out, `Category Name`=category_name) 
#%>%
...
category_phenotype_table %>% 
  ...
  write_tsv(file="tables/Table2_Classifier.tsv")
...
write(x="\n^=GyrA-87G and GyrA-87H are not included in QRDR count; qnrB1 is excluded from the single-PMQR count. *aac6 presence is not considered", file="tables/Table2_Classifier.tsv", append=T)
```
