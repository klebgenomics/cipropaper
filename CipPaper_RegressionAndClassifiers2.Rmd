...
rules_MIC_distribution_separate <- cipro_antibiogram %>%
  ...
  labs(x = "MIC (mg/L)", y = "Number of genomes", 
       ...
       title="MIC, by genotype profile", 
       fill="Genotype profile") +
...
ggsave(width=5, height=9, file="figs/FigX_MICdist_byProfile.pdf")
ggsave(width=5, height=9, file="figs/FigX_MICdist_byProfile.png")
...
discrete_quantile <- function(x, p) {
  sort(log2(x))[length(x)*p]
}


MIC_pred <- cipro_antibiogram %>%
  filter(Laboratory.Typing.Method != "Disk diffusion") %>% 
  group_by(rules_category) %>% 
  summarise(median=median(Measurement), 
            lower5=discrete_quantile(as.numeric(Measurement), p=0.05), 
            upper5=discrete_quantile(as.numeric(Measurement), p=0.95), 
            lower25=discrete_quantile(as.numeric(Measurement), p=0.25), 
            upper25=discrete_quantile(as.numeric(Measurement), p=0.75)) %>%
  mutate(ci90=paste0(median, " [", 2^lower5, "-", 2^upper5, "]")) %>%
  mutate(IQR=paste0(median, " [", 2^lower25, "-", 2^upper25, "]"))
