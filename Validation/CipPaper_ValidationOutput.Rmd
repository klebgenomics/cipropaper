...
```{r pooledR}
pooled_R <- 
bind_rows(EUSCAPE_EURECA_SIR, KayamaJapan_SIR, IndiaGHRU_SIR, AGAR_SIR,
Oxfordshire_Kpn_SIR, 
IZSAM_Italy_SIR, 
MBIRA_Kpn_SIR, 
HowdenGorrie_SIR, 
...
#bind_rows(LaPaz_Lazaro_genotype, IZSAM_Italy_genotype) %>%
#    group_by(Group) %>%
#   summarise_all(funs(sum(., na.rm = TRUE)))
  
pooled_genotype <- 
bind_rows(EUSCAPE_EURECA_genotype, KayamaJapan_genotype, IndiaGHRU_genotype, AGAR_genotype,
...
MBIRA_Kpn_excludeI <- MBIRA_Kpn %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

MBIRA_Kpn_excludeI_prediction <- matrix(c(subset(MBIRA_Kpn_excludeI, Group == "pred_S")$S, 
                subset(MBIRA_Kpn_excludeI, Group == "pred_S")$R,
                subset(MBIRA_Kpn_excludeI, Group == "pred_R")$S,
                subset(MBIRA_Kpn_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(MBIRA_Kpn_excludeI_prediction) <- c('S','R')
rownames(MBIRA_Kpn_excludeI_prediction) <- c('pred_S','pred_R')
...
MBIRA_Kvv_excludeI <- MBIRA_Kvv %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

MBIRA_Kvv_excludeI_prediction <- matrix(c(subset(MBIRA_Kvv_excludeI, Group == "pred_S")$S, 
                subset(MBIRA_Kvv_excludeI, Group == "pred_S")$R,
                subset(MBIRA_Kvv_excludeI, Group == "pred_R")$S,
                subset(MBIRA_Kvv_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(MBIRA_Kvv_excludeI_prediction) <- c('S','R')
rownames(MBIRA_Kvv_excludeI_prediction) <- c('pred_S','pred_R')
...
HowdenGorrie_excludeI <- HowdenGorrie %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

HowdenGorrie_excludeI_prediction <- matrix(c(subset(HowdenGorrie_excludeI, Group == "pred_S")$S, 
                subset(HowdenGorrie_excludeI, Group == "pred_S")$R,
                subset(HowdenGorrie_excludeI, Group == "pred_R")$S,
                subset(HowdenGorrie_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(HowdenGorrie_excludeI_prediction) <- c('S','R')
rownames(HowdenGorrie_excludeI_prediction) <- c('pred_S','pred_R')
...
AGAR_excludeI_prediction <- matrix(c(subset(AGAR_excludeI, Group == "pred_S")$S, 
                subset(AGAR_excludeI, Group == "pred_S")$R,
                subset(AGAR_excludeI, Group == "pred_R")$S,
                subset(AGAR_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(AGAR_excludeI_prediction) <- c('S','R')
rownames(AGAR_excludeI_prediction) <- c('pred_S','pred_R')
...
Milan_excludeI <- Milan %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

Milan_excludeI_prediction <- matrix(c(subset(Milan_excludeI, Group == "pred_S")$S, 
                subset(Milan_excludeI, Group == "pred_S")$R,
                subset(Milan_excludeI, Group == "pred_R")$S,
                subset(Milan_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(Milan_excludeI_prediction) <- c('S','R')
rownames(Milan_excludeI_prediction) <- c('pred_S','pred_R')
...
HailuEthiopia_excludeI <- HailuEthiopia %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

HailuEthiopia_excludeI_prediction <- matrix(c(subset(HailuEthiopia_excludeI, Group == "pred_S")$S, 
                subset(HailuEthiopia_excludeI, Group == "pred_S")$R,
                subset(HailuEthiopia_excludeI, Group == "pred_R")$S,
                subset(HailuEthiopia_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(HailuEthiopia_excludeI_prediction) <- c('S','R')
rownames(HailuEthiopia_excludeI_prediction) <- c('pred_S','pred_R')
...
KayamaJapan_excludeI <- KayamaJapan %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

KayamaJapan_excludeI_prediction <- matrix(c(subset(KayamaJapan_excludeI, Group == "pred_S")$S, 
                subset(KayamaJapan_excludeI, Group == "pred_S")$R,
                subset(KayamaJapan_excludeI, Group == "pred_R")$S,
                subset(KayamaJapan_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(KayamaJapan_excludeI_prediction) <- c('S','R')
rownames(KayamaJapan_excludeI_prediction) <- c('pred_S','pred_R')
...
FeaseyMalawi_excludeI <- FeaseyMalawi %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

FeaseyMalawi_excludeI_prediction <- matrix(c(subset(FeaseyMalawi_excludeI, Group == "pred_S")$S, 
                subset(FeaseyMalawi_excludeI, Group == "pred_S")$R,
                subset(FeaseyMalawi_excludeI, Group == "pred_R")$S,
                subset(FeaseyMalawi_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(FeaseyMalawi_excludeI_prediction) <- c('S','R')
rownames(FeaseyMalawi_excludeI_prediction) <- c('pred_S','pred_R')
...
EUSCAPE_EURECA_excludeI <- EUSCAPE_EURECA %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

EUSCAPE_EURECA_excludeI_prediction <- matrix(c(subset(EUSCAPE_EURECA_excludeI, Group == "pred_S")$S, 
                subset(EUSCAPE_EURECA_excludeI, Group == "pred_S")$R,
                subset(EUSCAPE_EURECA_excludeI, Group == "pred_R")$S,
                subset(EUSCAPE_EURECA_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(EUSCAPE_EURECA_excludeI_prediction) <- c('S','R')
rownames(EUSCAPE_EURECA_excludeI_prediction) <- c('pred_S','pred_R')
...
EgliSwitzerland_excludeI <- EgliSwitzerland %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

EgliSwitzerland_excludeI_prediction <- matrix(c(subset(EgliSwitzerland_excludeI, Group == "pred_S")$S, 
                subset(EgliSwitzerland_excludeI, Group == "pred_S")$R,
                subset(EgliSwitzerland_excludeI, Group == "pred_R")$S,
                subset(EgliSwitzerland_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(EgliSwitzerland_excludeI_prediction) <- c('S','R')
rownames(EgliSwitzerland_excludeI_prediction) <- c('pred_S','pred_R')
...
HRYC_Spain_excludeI <- HRYC_Spain %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

HRYC_Spain_excludeI_prediction <- matrix(c(subset(HRYC_Spain_excludeI, Group == "pred_S")$S, 
                subset(HRYC_Spain_excludeI, Group == "pred_S")$R,
                subset(HRYC_Spain_excludeI, Group == "pred_R")$S,
                subset(HRYC_Spain_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(HRYC_Spain_excludeI_prediction) <- c('S','R')
rownames(HRYC_Spain_excludeI_prediction) <- c('pred_S','pred_R')
...
IndiaGHRU_excludeI <- IndiaGHRU %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

IndiaGHRU_excludeI_prediction <- matrix(c(subset(IndiaGHRU_excludeI, Group == "pred_S")$S, 
                subset(IndiaGHRU_excludeI, Group == "pred_S")$R,
                subset(IndiaGHRU_excludeI, Group == "pred_R")$S,
                subset(IndiaGHRU_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(IndiaGHRU_excludeI_prediction) <- c('S','R')
rownames(IndiaGHRU_excludeI_prediction) <- c('pred_S','pred_R')
...
NeumannGermany_excludeI <- NeumannGermany %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

NeumannGermany_excludeI_prediction <- matrix(c(subset(NeumannGermany_excludeI, Group == "pred_S")$S, 
                subset(NeumannGermany_excludeI, Group == "pred_S")$R,
                subset(NeumannGermany_excludeI, Group == "pred_R")$S,
                subset(NeumannGermany_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(NeumannGermany_excludeI_prediction) <- c('S','R')
rownames(NeumannGermany_excludeI_prediction) <- c('pred_S','pred_R')
...
Whitelaw_CHAMPS_excludeI <- Whitelaw_CHAMPS %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

Whitelaw_CHAMPS_excludeI_prediction <- matrix(c(subset(Whitelaw_CHAMPS_excludeI, Group == "pred_S")$S, 
                subset(Whitelaw_CHAMPS_excludeI, Group == "pred_S")$R,
                subset(Whitelaw_CHAMPS_excludeI, Group == "pred_R")$S,
                subset(Whitelaw_CHAMPS_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(Whitelaw_CHAMPS_excludeI_prediction) <- c('S','R')
rownames(Whitelaw_CHAMPS_excludeI_prediction) <- c('pred_S','pred_R')
...
Oxfordshire_Kpn_excludeI <- Oxfordshire_Kpn %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

Oxfordshire_Kpn_excludeI_prediction <- matrix(c(subset(Oxfordshire_Kpn_excludeI, Group == "pred_S")$S, 
                subset(Oxfordshire_Kpn_excludeI, Group == "pred_S")$R,
                subset(Oxfordshire_Kpn_excludeI, Group == "pred_R")$S,
                subset(Oxfordshire_Kpn_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(Oxfordshire_Kpn_excludeI_prediction) <- c('S','R')
rownames(Oxfordshire_Kpn_excludeI_prediction) <- c('pred_S','pred_R')
...
Oxfordshire_Kvv_excludeI <- Oxfordshire_Kvv %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

Oxfordshire_Kvv_excludeI_prediction <- matrix(c(subset(Oxfordshire_Kvv_excludeI, Group == "pred_S")$S, 
                subset(Oxfordshire_Kvv_excludeI, Group == "pred_S")$R,
                subset(Oxfordshire_Kvv_excludeI, Group == "pred_R")$S,
                subset(Oxfordshire_Kvv_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(Oxfordshire_Kvv_excludeI_prediction) <- c('S','R')
rownames(Oxfordshire_Kvv_excludeI_prediction) <- c('pred_S','pred_R')
...
IZSAM_Italy_excludeI <- IZSAM_Italy %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

IZSAM_Italy_excludeI_prediction <- matrix(c(subset(IZSAM_Italy_excludeI, Group == "pred_S")$S, 
                subset(IZSAM_Italy_excludeI, Group == "pred_S")$R,
                subset(IZSAM_Italy_excludeI, Group == "pred_R")$S,
                subset(IZSAM_Italy_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(IZSAM_Italy_excludeI_prediction) <- c('S','R')
rownames(IZSAM_Italy_excludeI_prediction) <- c('pred_S','pred_R')
...
LaPaz_Lazaro_excludeI <- LaPaz_Lazaro %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

LaPaz_Lazaro_excludeI_prediction <- matrix(c(subset(LaPaz_Lazaro_excludeI, Group == "pred_S")$S, 
                subset(LaPaz_Lazaro_excludeI, Group == "pred_S")$R,
                subset(LaPaz_Lazaro_excludeI, Group == "pred_R")$S,
                subset(LaPaz_Lazaro_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(LaPaz_Lazaro_excludeI_prediction) <- c('S','R')
rownames(LaPaz_Lazaro_excludeI_prediction) <- c('pred_S','pred_R')
...
HarrisMERINO_excludeI <- HarrisMERINO %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

HarrisMERINO_excludeI_prediction <- matrix(c(subset(HarrisMERINO_excludeI, Group == "pred_S")$S, 
                subset(HarrisMERINO_excludeI, Group == "pred_S")$R,
                subset(HarrisMERINO_excludeI, Group == "pred_R")$S,
                subset(HarrisMERINO_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(HarrisMERINO_excludeI_prediction) <- c('S','R')
rownames(HarrisMERINO_excludeI_prediction) <- c('pred_S','pred_R')
...
PachecoBrazil_excludeI <- PachecoBrazil %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_R") %>%
  rowwise()%>%
  select(-c(I)) 

PachecoBrazil_excludeI_prediction <- matrix(c(subset(PachecoBrazil_excludeI, Group == "pred_S")$S, 
                subset(PachecoBrazil_excludeI, Group == "pred_S")$R,
                subset(PachecoBrazil_excludeI, Group == "pred_R")$S,
                subset(PachecoBrazil_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(PachecoBrazil_excludeI_prediction) <- c('S','R')
rownames(PachecoBrazil_excludeI_prediction) <- c('pred_S','pred_R')
...
```{r pooled_excludeI}
pooled_excludeI <- 
bind_rows(EUSCAPE_EURECA_excludeI, KayamaJapan_excludeI, IndiaGHRU_excludeI, AGAR_excludeI,
Oxfordshire_Kpn_excludeI, 
IZSAM_Italy_excludeI, 
MBIRA_Kpn_excludeI, 
HowdenGorrie_excludeI, 
Milan_excludeI,
HRYC_Spain_excludeI,
LaPaz_Lazaro_excludeI,
MBIRA_Kvv_excludeI, 
HarrisMERINO_excludeI,
PachecoBrazil_excludeI) %>%
    group_by(Group) %>%
    summarise_all(funs(sum(., na.rm = TRUE)))

pooled_excludeI_prediction <- matrix(c(subset(pooled_excludeI, Group == "pred_S")$S, 
                subset(pooled_excludeI, Group == "pred_S")$R,
                subset(pooled_excludeI, Group == "pred_R")$S,
                subset(pooled_excludeI, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(pooled_excludeI_prediction) <- c('S','R')
rownames(pooled_excludeI_prediction) <- c('pred_S','pred_R')
...
# supplementary table with all metrics and datasets
```{r}
...
write_tsv(all_datasets_metrics, "../tables/TableS9_extValidation_evalMetrics.tsv", na="-")
```
...
```{r summary_validation, eval=FALSE}
