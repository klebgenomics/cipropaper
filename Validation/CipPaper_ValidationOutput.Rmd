...
Whitelaw_MBIRA <- read_delim("Whitelaw_Stellenbosch_ValidationOutput/Prediction_Pheno_summary_MIC.csv")
...
# data wrangling EgliSwitzerland
```{r}
...
# data wrangling Whitelaw_CHAMPS
```{r Whitelaw_CHAMPS}
...
# data wrangling Whitelaw_MBIRA
```{r Whitelaw_MBIRA}

Whitelaw_MBIRA_SI <- Whitelaw_MBIRA %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_S" | Group=="pred_I") %>%
  rowwise()%>%
  mutate(S_I = sum(S, I)) %>%
...
Whitelaw_MBIRA_R <- Whitelaw_MBIRA %>% 
  rename(Group=...1) %>% 
  filter(Group=="pred_R" | Group=="pred_I") %>%
  rowwise()%>%
  mutate(S_I = sum(S, I)) %>%
...
Whitelaw_MBIRA_SIR <- rbind(Whitelaw_MBIRA_SI, Whitelaw_MBIRA_R) %>% distinct()

subset(Whitelaw_MBIRA_SIR, Group == "pred_S_I")$S_I
table(Whitelaw_MBIRA_SIR$S_I)

Whitelaw_MBIRA_R_prediction <- matrix(c(subset(Whitelaw_MBIRA_SIR, Group == "pred_S_I")$S_I, 
                subset(Whitelaw_MBIRA_SIR, Group == "pred_S_I")$R,
                subset(Whitelaw_MBIRA_SIR, Group == "pred_R")$S_I,
                subset(Whitelaw_MBIRA_SIR, Group == "pred_R")$R), ncol=2, byrow=TRUE)

colnames(Whitelaw_MBIRA_R_prediction) <- c('S_I','R')
rownames(Whitelaw_MBIRA_R_prediction) <- c('pred_S_I','pred_R')
Whitelaw_MBIRA_R_prediction_summary<-metrics(Whitelaw_MBIRA_R_prediction)

Whitelaw_MBIRA_NWT_prediction <- matrix(c(subset(Whitelaw_MBIRA_SIR, Group == "pred_S")$S, 
                subset(Whitelaw_MBIRA_SIR, Group == "pred_S")$I_R,
                subset(Whitelaw_MBIRA_SIR, Group == "pred_I_R")$S,
                subset(Whitelaw_MBIRA_SIR, Group == "pred_I_R")$I_R), ncol=2, byrow=TRUE)
colnames(Whitelaw_MBIRA_NWT_prediction) <- c('S','I_R')
rownames(Whitelaw_MBIRA_NWT_prediction) <- c('pred_S','pred_I_R')
Whitelaw_MBIRA_NWT_prediction_summary<-metrics(Whitelaw_MBIRA_NWT_prediction)
```
...
validation_summary <- rbind(EUSCAPE_EURECA_R_prediction_summary$summary,
                            ...
                            MBIRA_R_prediction_summary$summary,
                            ...
                            Whitelaw_MBIRA_R_prediction_summary$summary, 
                            Whitelaw_MBIRA_NWT_prediction_summary$summary,                            
...
validation_summary_table <- as_tibble(validation_summary) %>%
  mutate(Prediction=c("R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT", "R", "NWT", "R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT", "R", "NWT"))%>%
  mutate(Dataset=c("EUSCAPE & EURECA", "EUSCAPE & EURECA","JARBS-GNR", "JARBS-GNR", "India GHRU", "India GHRU", "AGAR GnSOP", "AGAR GnSOP","MBIRA", "MBIRA", "Controlling Superbugs study & Victorian CPE program", "Controlling Superbugs study & Victorian CPE program",  "IRCCS Ospedale San Raffaele", "IRCCS Ospedale San Raffaele",  "HURYC & CIBERINFEC", "HURYC & CIBERINFEC", "SCHARKI", "SCHARKI","MBRIA","MBIRA", "Queen Elizabeth Central Hospital", "Queen Elizabeth Central Hospital","University of Zurich", "University of Zurich", "Addis Ababa University", "Addis Ababa University", "CHAMPS", "CHAMPS")) %>%
    mutate(Country=c("Various European countries", "Various European countries","Japan", "Japan", "India", "India", "Australia", "Australia","Tanzania, Ethiopia, Malawi, Zambia, Ghana, Nigeria, South Africa", "Tanzania, Ethiopia, Malawi, Zambia, Ghana, Nigeria, South Africa", "Australia", "Australia",  "Italy", "Italy",  "Spain", "Spain", "Germany", "Germany","South Africa","South Africa", "Malawi", "Malawi","Switzerland", "Switzerland", "Ethiopia", "Ethiopia", "South Africa", "South Africa")) %>%
  ...
  mutate(`Categorical agreement`=paste0(round(as.numeric(`Categorical agreement`)*100,2),"%")) %>%  select(Prediction, Dataset, Country, `Categorical agreement`, Sensitivity, Specificity, ME, VME, N)
...
Whitelaw_MBIRA_MIC <- readPNG("Whitelaw_Stellenbosch_ValidationOutput/MIC_PredRIS.png", native=TRUE)
...
patch <- (wrap_elements(EUSCAPE_EURECA_MIC) +
            ...
            ggtitle("E) MBIRA MIC (n=228)")+             
            wrap_elements(HowdenGorrie_MIC) +
            ggtitle("F) Controlling Superbugs study & \nVictorian CPE program MIC (n=212)") +
            wrap_elements(Milan_MIC) +
            ggtitle("G) IRCCS Ospedale San Raffaele MIC (n=198)")+
            wrap_elements(HRYC_Spain_MIC) +
            ggtitle("H) HURYC & CIBERINFEC MIC (n=173)")+
            wrap_elements(NeumannGermany_MIC) +
            ggtitle("I) SCHARKI MIC (n=56)")+  
             wrap_elements(Whitelaw_MBIRA_MIC) +
            ggtitle("J) MBIRA (South Africa) MIC (n=56)")+             
            wrap_elements(FeaseyMalawi_DD) +
            ggtitle("K) Queen Elizabeth Central Hospital \n(Malawi) DD (n=737)") +
            wrap_elements(EgliSwitzerland_DD) +
            ggtitle("L) University of Zurich DD (n=72)")+
            wrap_elements(HailuEthiopia_DD) +
            ggtitle("M) Addis Ababa University \n(Ethiopia) DD (n=70)")+
            wrap_elements(Whitelaw_CHAMPS_DD) +
            ggtitle("N) CHAMPS DD (n=40)")
          )+
  plot_layout(ncol = 3)
...
WhitelawMBIRA_dist <- readPNG("Whitelaw_Stellenbosch_ValidationOutput/MIC_PredGroup.png", native=TRUE)
WhitelawMBIRA_profile <- readPNG("Whitelaw_Stellenbosch_ValidationOutput/StackedBar_PredGroup_SIR_MIC.png", native=TRUE)    
...
(wrap_elements(MBIRA_MIC_dist) +ggtitle("I) MBIRA (n=228) - MIC distribution vs.\n genotype profile") + wrap_elements(MBIRA_profile) + ggtitle("J) MBIRA (n=228) - observed phenotype vs.\n genotype profile")) /

(wrap_elements(HowdenGorrie_dist) +ggtitle("K) Controlling Superbugs study & \nVictorian CPE program (n=212) - MIC distribution vs.\n genotype profile") + wrap_elements(HowdenGorrie_profile) + ggtitle("L) Controlling Superbugs study & \nVictorian CPE program (n=212) - observed phenotype vs.\n genotype profile")) /
  
(wrap_elements(Milan_dist) +ggtitle("M) IRCCS Ospedale San Raffaele (n=198) - MIC distribution vs.\n genotype profile") + wrap_elements(Milan_profile) + ggtitle("N) IRCCS Ospedale San Raffaele (n=198) \n - observed phenotype vs. genotype profile")) /
  
  (wrap_elements(HRYC_Spain_dist) +ggtitle("O) HURYC & CIBERINFEC (n=173) - MIC distribution vs.\n genotype profile") + wrap_elements(HRYC_Spain_profile) + ggtitle("P) HURYC & CIBERINFEC (n=173) \n - observed phenotype vs. genotype profile")) 
...
ggsave(height=13, width=10, file="../figs/FigS11_extValidation_genoprofiles_2.pdf")
    ...
    (wrap_elements(NeumannGermany_dist) +ggtitle("Q) SCHARKI (n=56) - MIC distribution vs.\n genotype profile") + wrap_elements(NeumannGermany_profile) + ggtitle("R) SCHARKI (n=56) - observed phenotype vs.\n genotype profile")) /

    (wrap_elements(WhitelawMBIRA_dist) +ggtitle("S) MBIRA (South Africa) (n=56) - MIC distribution vs.\n genotype profile") + wrap_elements(WhitelawMBIRA_profile) + ggtitle("T) MBIRA (South Africa) (n=56) - observed phenotype vs.\n genotype profile")) /


(wrap_elements(Feasey_dist) +ggtitle("U) Queen Elizabeth Central Hospital Malawi (n=737) \n - DD distribution vs. genotype profile") + wrap_elements(Feasey_profile) + ggtitle("V) Queen Elizabeth Central Hospital Malawi (n=737) \n - observed phenotype vs. genotype profile")) /
  
  (wrap_elements(EgliSwitzerland_dist) +ggtitle("W) University of Zurich (n=72) - DD distribution vs.\n genotype profile") + wrap_elements(EgliSwitzerland_profile) + ggtitle("X) University of Zurich (n=72) - observed phenotype vs.\n genotype profile")) 
...
(wrap_elements(Hailu_dist) +ggtitle("Y) Addis Ababa University (n=70) - DD distribution vs.\n genotype profile") + wrap_elements(Hailu_profile) + ggtitle("Z) Addis Ababa University (n=70) - observed phenotype vs.\n genotype profile")) /

(wrap_elements(WhitelawCHAMPS_dist) +ggtitle("AA) CHAMPS (n=40) - DD distribution vs.\n genotype profile") + wrap_elements(WhitelawCHAMPS_profile) + ggtitle("BB) CHAMPS (n=40) - observed phenotype vs.\n genotype profile")) 
