...
validation_summary_table <- as_tibble(validation_summary) %>%
  mutate(Prediction=c("R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT", "R", "NWT", "R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT","R", "NWT", "R", "NWT","R", "NWT","R", "NWT"))%>%
...
colSums(SIR_table[,-1])
validation_summary_table_total$Dataset_label[validation_summary_table_total$Dataset_label == 'All datasets combined (S=NA, R=NA)'] <- "All datasets combined (S=1695, R=4464)"
...
validation_summary_table_total$Dataset_label <- factor(as.character(validation_summary_table_total$Dataset_label), levels = c("A (EUCAST) (S=291, R=1687)", "B (EUCAST) (S=84, R=441)","C (EUCAST) (S=168, R=255)", "D (EUCAST) (S=242, R=37)", "E (EUCAST) (S=20, R=146)", "F (EUCAST) (S=11, R=136)", "G (CLSI) (S=222, R=449)","H (CLSI) (S=109, R=128)", "I (CLSI) (S=18, R=153)", "J* (EUCAST/BSAC) (S=196, R=411)","K* (Kvv, CLSI) (S=37, R=28)","L* (EUCAST) (S=11, R=53)", "M* (CLSI) (S=18, R=21)", "IZSAM (S=7, R=273)",  "IRCCS Ospedale San Raffaele (S=9, R=187)", "MBIRA (Kvv, CLSI) (S=85, R=5)",  "Oxfordshire (Kvv, EUCAST) (S=64, R=0)", "SCHARKI (S=51, R=2)","EUSCAPE & EURECA (Kvv, EUCAST) (S=35, R=3)", "PachecoBrazil (S=10, R=28)", "HarrisMERINO (S=7, R=21)", "Overall median (datasets A-M)", "All datasets combined (S=1695, R=4464)"))
...
validation_summary_table_total_exclude %>% group_by(`Evaluation metric`) %>% 
  ...
  mutate(Dataset="Overall median (datasets A-M)") %>%
  mutate(var="") %>%
  mutate(ub="") %>%
  mutate(lb="") %>%
  mutate(Country="") %>%
  ...
  mutate(`R_total`="")%>%
  mutate(`N_total`="")%>%
  mutate(`Dataset_label`="Overall median (datasets A-M)")
...
#write_tsv(all_datasets_metrics, "../tables/TableS11_extValidation_evalMetrics.tsv", na="-")
...
validation_summary_table_total$Dataset_label[validation_summary_table_total$Dataset_label == 'Overall median (S=NA, I=NA, R=NA)'] <- 'Overall median (datasets A-M)'
...
(wrap_elements(Hailu_dist) +ggtitle("21) K* (Kvv, CLSI) / \n Addis Ababa University* (n=70) .\n - DD distribution vs. genotype profile") + wrap_elements(Hailu_profile) + ggtitle("22) K* (Kvv, CLSI) / \n Addis Ababa University* (n=70) \n - observed phenotype vs. genotype profile")) /

    (wrap_elements(EgliSwitzerland_dist) +ggtitle("23) L* (EUCAST) / \n University of Zurich (n=72) - \n DD distribution vs. genotype profile") + wrap_elements(EgliSwitzerland_profile) + ggtitle("24) L* (EUCAST) / \n University of Zurich (n=72) \n - observed phenotype vs. genotype profile")) 
...
(wrap_elements(WhitelawCHAMPS_dist) +ggtitle("25) M* (CLSI) / \n CHAMPS (n=40) - DD distribution vs.\n genotype profile") + wrap_elements(WhitelawCHAMPS_profile) + ggtitle("26) M* (CLSI) / \n CHAMPS (n=40) - observed phenotype vs.\n genotype profile")) /
  
  (wrap_elements(IZSAM_Italy_dist) +ggtitle("27) IZSAM (n=283) - MIC distribution vs.\n genotype profile") + wrap_elements(IZSAM_Italy_profile) + ggtitle("28) IZSAM (n=283) - observed phenotype vs.\n genotype profile")) 
...
(wrap_elements(Milan_dist) +ggtitle("29) IRCCS Ospedale San Raffaele (n=198) -\n MIC distribution vs. genotype profile") + wrap_elements(Milan_profile) + ggtitle("30) IRCCS Ospedale San Raffaele (n=198) \n - observed phenotype vs. genotype profile")) /

(wrap_elements(MBIRA_Kvv_dist) +ggtitle("31) MBIRA (Kvv, CLSI) (n=92) - MIC distribution vs.\n genotype profile") + wrap_elements(MBIRA_Kvv_profile) + ggtitle("32) MBIRA (Kvv, CLSI) (n=92) - observed phenotype vs.\n genotype profile")) 
...
(wrap_elements(Oxfordshire_Kvv_dist) +ggtitle("33) Oxfordshire (Kvv, EUCAST) (n=66) \n - MIC distribution vs. genotype profile") + wrap_elements(Oxfordshire_Kvv_profile) + ggtitle("34) Oxfordshire (Kvv, EUCAST) (n=66) \n - observed phenotype vs. genotype profile")) /

(wrap_elements(NeumannGermany_dist) +ggtitle("35) SCHARKI (n=56) - MIC distribution vs.\n genotype profile") + wrap_elements(NeumannGermany_profile) + ggtitle("36) SCHARKI (n=56) - observed phenotype vs.\n genotype profile")) 
...
(wrap_elements(EUSCAPE_EURECA_Kvv_dist) +ggtitle("37) EUSCAPE/EURECA Kvv (n=41) \n - MIC distribution vs. genotype profile") + wrap_elements(EUSCAPE_EURECA_Kvv_profile) + ggtitle("38) EUSCAPE/EURECA Kvv (n=41) \n - observed phenotype vs. genotype profile"))/

(wrap_elements(Pacheco_Brazil_dist) +ggtitle("39) iberoAMRica study (n=38) \n - MIC distribution vs. genotype profile") + wrap_elements(Pacheco_Brazil_profile) + ggtitle("40) iberoAMRica study (n=38) \n - observed phenotype vs. genotype profile")) 
...
ggsave(height=13, width=11, file="../figs/FigS11_extValidation_genoprofiles_10.pdf")
  ...
  (wrap_elements(Harris_MERINO_dist) +ggtitle("41) MERINO trial (n=36) \n - DD distribution vs. genotype profile") + wrap_elements(Harris_MERINO_dist) + ggtitle("42) MERINO trial (n=36) \n - observed phenotype vs. genotype profile")) 



ggsave(height=5, width=11, file="../figs/FigS11_extValidation_genoprofiles_11.png")
ggsave(height=5, width=11, file="../figs/FigS11_extValidation_genoprofiles_11.pdf")
