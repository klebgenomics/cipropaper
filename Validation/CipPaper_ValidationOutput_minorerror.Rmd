...
validation_summary_table <- as_tibble(validation_summary) %>%
  ...
  mutate(
    ...
    `Minor error` = paste0(round(as.numeric(`Minor error`) * 100, 2), "%"),
...
validation_summary_table_plot <- validation_summary_table %>% 
  filter(Prediction =="R") %>%
  relocate(Dataset, Prediction, `Categorical agreement`, `Sensitivity`, `Specificity`, `ME`, `VME`, `Minor error`)

validation_summary_table_long <- gather(
  validation_summary_table_plot,
  `Evaluation metric`,
  value,
  `Categorical agreement`:`Minor error`
)
...
validation_summary_table_long_ci <- 
  ...
  filter(
    ...
    `Evaluation metric` == "Minor error" & var == "Minerror" |
