...
major_error <- function(xtabs3) {

  xt3 <- as.matrix(xtabs3)
  storage.mode(xt3) <- "numeric"

  major_n <- xt3["pred_R", "S"]

  susceptible_n <-
  xt3["pred_S", "S"] +
  xt3["pred_R", "S"]

  p <- ifelse(susceptible_n == 0,
              NA_real_,
              major_n / susceptible_n)

  ci <- prop_ci(p, susceptible_n)

  list(
    me = p,
    me_lb = unname(ci["lower"]),
    me_ub = unname(ci["upper"])
  )
}

very_major_error <- function(xtabs3) {
  ...
  p <- ifelse(resistant_n == 0,
              NA_real_,
              vme_n / resistant_n)

  ci <- prop_ci(p, resistant_n)

  list(
    vme = p,
    vme_lb = unname(ci["lower"]),
    vme_ub = unname(ci["upper"])
  )
}
...
metrics <- function(xtabs, xtabs3 = NULL) {
  ...
  me_out  <- major_error(xtabs3)
  vme_out <- very_major_error(xtabs3)
  ...
  vme_ci <- c(
    lower = vme_out$vme_lb,
    upper = vme_out$vme_ub
  )
  ...
  me_ci <- c(lower = NA_real_, upper = NA_real_)
  vme_ci <- c(lower = NA_real_, upper = NA_real_)
}
