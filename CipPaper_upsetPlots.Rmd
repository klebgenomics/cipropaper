...
# Supp Fig 5 - number of QRDR vs DD, facet aac6 yes/no (no other genes)
``` {r}
...
# Supp Fig 3 panel a - upset plot of QRDR positions, absence of aac6/acquired genes, with MIC data
``` {r}
...
# Supp Fig 3 panel b - upset plot of QRDR positions, presence of aac6 but no other acquired genes, with MIC data
``` {r}
...
# Supp Fig 4 panel a - QRDR positions, absence of aac6/acquired genes, with DD data
``` {r}
...
# Supp Fig 4 panel b - QRDR positions, presence of aac6 but no other acquired genes, with DD data
``` {r}
...
ggsave("figs/Fig2_numQnr_MIC_pheno.pdf", width=6, height=5)
ggsave("figs/Fig2_numQnr_MIC_pheno.png", width=6, height=5)
...
## set up binary variables for common genes
``` {r}
...
qnr_genes <- cipro_res_mech %>% 
  ...
  count() %>% filter(startsWith(Resistance.mech.type, "q")) %>% 
  filter(n>10) %>% pull(Resistance.mech.type)

qnr_presence <- cipro_res_mech %>% 
  mutate(Resistance.mech.type=gsub("[\\^\\*\\?]", "", Resistance.mech.type)) %>%
  mutate(Resistance.mech.type=gsub(".v1", "", Resistance.mech.type)) %>%
  mutate(Resistance.mech.type=gsub(".v2", "", Resistance.mech.type)) %>%
  filter(Resistance.mech.type %in% qnr_genes) %>%
  group_by(strain, Resistance.mech.type) %>% count() %>% pivot_wider(names_from=Resistance.mech.type, values_from=n)
...
# Fig S6 panel - upset plot of qnr genes, absence of aac6 and QRDR, with MIC data
``` {r}
...
# (Fig S6-7) Supp Figs: qnr genes vs pheno (MIC & DD, with/without aac6)
```{r}

upset_qnr_noAac6_DD | upset_qnr_aac6_DD

ggsave("figs/Fig7_qnr_DD.pdf", width=12, height=6)
ggsave("figs/Fig7_qnr_DD.png", width=12, height=6)


upset_qnr_noAac6_MIC | upset_qnr_aac6_MIC

ggsave("figs/Fig6_qnr_MIC.pdf", width=12, height=6)
ggsave("figs/Fig6_qnr_MIC.png", width=12, height=6)

```

# Fig3 panel - aac6, QRDR mutations, qnr with DD data
``` {r}
...
# Fig3 panel - aac6, QRDR mutations, qnr with MIC data
``` {r}
...
# effect of aac6 gene, in absence of QRDR and other acquired

``` {r}

# MIC
aac_MIC_nullBG_dat <- mic %>%
...
t.test(log2(as.numeric(Measurement)) ~ aac6, data=aac_MIC_nullBG_dat)
...
t.test(as.numeric(Measurement) ~ aac6, data=aac_DD_nullBG_dat)
