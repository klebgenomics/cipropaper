...
# Quality control 
kleborate<-kleborate%>%
  filter(species_match=="strong") %>%
  filter(contig_count<=500)%>% 
  filter(total_size<=6132846 & total_size>=4969898)
...
rules_MIC_distributionRS <- cipro_antibiogram %>%
  ...
  labs(title = "", y = "Number of genomes", x="MIC (mg/L)", 
...
rules_DD_distributionRS <- cipro_antibiogram %>%
  ...
  labs(title = "", y = "Number of genomes", x ="Disk zone (mm)",
...
rules_MIC_distribution <- cipro_antibiogram %>%
  ...
  labs(x = "MIC (mg/L)", y = "Number of genomes", 
       linetype = "MIC breakpoints", title="", fill="Genotype profile") +
...
rules_DD_distribution <- cipro_antibiogram %>%
 ...
 # geom_vline(aes(xintercept = 21.5), color = "black") +
  ...
  theme_minimal()+
  labs(y = "Number of genomes", x="Disk zone (mm)", title="") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, hjust=1, size=20),
        axis.text.y = element_text(size=20),
        axis.title.y = element_text(size=20),
