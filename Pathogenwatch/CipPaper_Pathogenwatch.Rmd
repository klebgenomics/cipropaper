...
#Pathogenwatch data downloaded Aug 11, 2025

Kp <- read.delim(file = "Klebsiella pneumoniae__kleborate.csv.gz", header = TRUE, sep = ",")
Kp <- read.delim(file = "Klebsiella pneumoniae__metadata.csv.gz", header = TRUE, sep = ",") %>% 
  rename(`Genome.ID`="id") %>% full_join(Kp,by="Genome.ID")
Kp <- read.delim(file = "Klebsiella pneumoniae__klebsiella-lincodes.csv.gz", header = TRUE, sep = ",") %>%  full_join(Kp,by="Genome.ID")

Ka <- read.delim(file = "Klebsiella africana__kleborate.csv.gz", header = TRUE, sep = ",")
Ka <- read.delim(file = "Klebsiella africana__metadata.csv.gz", header = TRUE, sep = ",") %>% 
  rename(`Genome.ID`="id") %>% full_join(Ka,by="Genome.ID")
Ka <- read.delim(file = "Klebsiella africana__klebsiella-lincodes.csv.gz", header = TRUE, sep = ",") %>%  full_join(Ka,by="Genome.ID")

Kqp <- read.delim(file = "Klebsiella quasipneumoniae__kleborate.csv.gz", header = TRUE, sep = ",")
Kqp <- read.delim(file = "Klebsiella quasipneumoniae__metadata.csv.gz", header = TRUE, sep = ",")%>% 
  rename(`Genome.ID`="id") %>% full_join(Kqp,by="Genome.ID")
Kqp <- read.delim(file = "Klebsiella quasipneumoniae__klebsiella-lincodes.csv.gz", header = TRUE, sep = ",") %>%  full_join(Kqp,by="Genome.ID")


Kqv <- read.delim(file = "Klebsiella quasivariicola__kleborate.csv.gz", header = TRUE, sep = ",")
Kqv <- read.delim(file = "Klebsiella quasivariicola__metadata.csv.gz", header = TRUE, sep = ",")%>% 
  rename(`Genome.ID`="id") %>% full_join(Kqv,by="Genome.ID")
Kqv <- read.delim(file = "Klebsiella quasivariicola__klebsiella-lincodes.csv.gz", header = TRUE, sep = ",") %>%  full_join(Kqv,by="Genome.ID")

Kv <- read.delim(file = "Klebsiella variicola__kleborate.csv.gz", header = TRUE, sep = ",")
Kv <- read.delim(file = "Klebsiella variicola__metadata.csv.gz", header = TRUE, sep = ",")%>% 
  rename(`Genome.ID`="id") %>% full_join(Kv,by="Genome.ID")
Kv <- read.delim(file = "Klebsiella variicola__klebsiella-lincodes.csv.gz", header = TRUE, sep = ",") %>%  full_join(Kv,by="Genome.ID")

pathogenwatch <- bind_rows(Kp %>% select(Flq_acquired, Flq_mutations, Ciprofloxacin_profile, Ciprofloxacin_MIC_prediction,  year, Country, ST, species, Study.accession, LIN.code),
                Ka %>% select(Flq_acquired, Flq_mutations, year, Country, Ciprofloxacin_profile, Ciprofloxacin_MIC_prediction, ST, species, Study.accession, LIN.code),
                Kqv %>% select(Flq_acquired, Flq_mutations, year, Country, Ciprofloxacin_profile, Ciprofloxacin_MIC_prediction, ST, species, Study.accession, LIN.code),
                Kv %>% select(Flq_acquired, Flq_mutations, year, Country, Ciprofloxacin_profile, Ciprofloxacin_MIC_prediction, ST, species, Study.accession, LIN.code),
                Kqp %>% select(Flq_acquired, Flq_mutations, year, Country, Ciprofloxacin_profile, Ciprofloxacin_MIC_prediction, ST, species, Study.accession, LIN.code))
...
pathogenwatch <- pathogenwatch %>%
  ...
  mutate(`GyrA-83` = str_count(Flq_mutations, "GyrA:p.Asp83")) %>%
  mutate(`GyrA-87` = str_count(Flq_mutations, "GyrA:p.Asp87")) %>%
  mutate(`ParC-80` = str_count(Flq_mutations, "ParC:p.Ser80")) %>% 
  mutate(`ParC-84` = str_count(Flq_mutations, "ParC:p.Ser84")) %>%
 ...
 mutate(aac6 = str_count(Flq_acquired, "aac")) %>%
...
pathogenwatch <- pathogenwatch %>% 
  mutate(QRDR_mutations_exclude = str_count(Flq_mutations, "GyrA:p.Asp87Gly") + str_count(Flq_mutations, "GyrA:p.Asp87His")) %>% 
  mutate(PMQR_exclude = str_count(Flq_acquired, "qnrB1.v")) %>%
  mutate(rules_category = case_when(
    Ciprofloxacin_profile=="0^ QRDR, 0 PMQR, 0 aac(6`)-Ib-cr" ~ 0, # no determinants -> WT S
    Ciprofloxacin_profile=="0 QRDR, 0 PMQR, 1 aac(6`)-Ib-cr" ~ 1, # aac6 solo -> WT S
    Ciprofloxacin_profile=="0 QRDR, qnrB1, 0 aac(6`)-Ib-cr" ~ 2, # non-associated PMQR solo -> NWT I
    Ciprofloxacin_profile=="1 QRDR, 0 PMQR, 0 aac(6`)-Ib-cr" ~ 3, # 1 QRDR, except those not associated -> NWT R
    Ciprofloxacin_profile=="1 QRDR, 0 PMQR, 1 aac(6`)-Ib-cr" ~ 4, # 1 QRDR + aac6 -> NWT R
    Ciprofloxacin_profile==">1 QRDR, 0 PMQR, * aac(6`)-Ib-cr" ~ 5, # ≥2 QRDR -> NWT R
    Ciprofloxacin_profile=="0 QRDR, 1^ PMQR, 0 aac(6`)-Ib-cr" ~ 6, # 1 PMQR, except those not associated -> NWT R
    Ciprofloxacin_profile=="0 QRDR, 1 PMQR, 1 aac(6`)-Ib-cr" ~ 7, # 1 PMQR + aac6
    Ciprofloxacin_profile=="0 QRDR, >1 PMQR, * aac(6`)-Ib-cr" ~ 8, # ≥2 PMQR -> NWT R
    Ciprofloxacin_profile==">0 QRDR, >0 PMQR, * aac(6`)-Ib-cr" ~ 9, # QRDR + PMQR -> NWT R
    TRUE ~ NA
  )) %>%
...
# total genomes before filtering
nrow(pathogenwatch)

PW <- pathogenwatch

# exclude LIN code = NA, country=NA
PW <- PW[!(is.na(PW$LIN.code) | PW$LIN.code==""), ]
PW <- PW[!(is.na(PW$Country)| PW$Country==""), ]
PW <- PW[!(is.na(PW$Study.accession)| PW$Study.accession==""), ]
...
# total genomes after filtering 
nrow(PW)

# remove redundancy by study accession, country, year, LIN.code, rules_category
PW<-PW[!duplicated(PW[,c('Study.accession','Country', 'year', 'LIN.code','rules_category' )]),]

#genomes, country, and year included in downstream analysis
nrow(PW)
length(unique(PW$Country))
length(unique(PW$year))
...
PW %>% group_by(ST, QRDR_mutations) %>% filter(ST=="ST258"|ST=="ST11"|ST=="ST307"|ST=="ST147"|ST=="ST15") %>% count(sort=TRUE)
...
PW %>% group_by(ST, acquired_genes) %>% filter(ST=="ST258")%>% count(sort=TRUE)
PW %>% group_by(ST, aac6) %>% filter(ST=="ST258")%>% count(sort=TRUE)

PW %>% group_by(ST, acquired_genes) %>% filter(ST=="ST11")%>% count(sort=TRUE)
PW %>% group_by(ST, acquired_genes) %>% filter(ST=="ST307")%>% count(sort=TRUE)
PW %>% group_by(ST, acquired_genes) %>% filter(ST=="ST147")%>% count(sort=TRUE)
PW %>% group_by(ST, acquired_genes) %>% filter(ST=="ST15")%>% count(sort=TRUE)
...
PW %>% filter(ST=="ST347") %>% group_by(rules_category) %>% count()

PW %>% filter(species=="Klebsiella variicola subsp. variicola") %>% group_by(ST) %>% count(sort=TRUE) 

PW %>% filter(ST=="ST641"|ST=="ST355"|ST=="ST697"|ST=="ST1562"|ST=="ST197"|ST=="ST919") %>% group_by(ST, rules_category) %>% count()
...
PW_dat_variant_kp <- PW %>% 
  filter(species=="Klebsiella pneumoniae") %>%
  mutate(variant=paste(Flq_acquired, Flq_mutations, sep = ";")) %>%
...
PW_dat_variant_kp$variant[PW_dat_variant_kp$variant == 'QnrB76'] <- 'qnrB76'
PW_dat_variant_kp$variant[PW_dat_variant_kp$variant == 'QnrB77'] <- 'qnrB77'

PW_dat_variant_kp$ST <- gsub("ST", "", PW_dat_variant_kp$ST)

ST_freq$ST <- gsub("ST", "", ST_freq$ST)
...
# Figure S12 - PW determinants vs ST
...
ggsave(height=9, width=8, file="../figs/FigureS12_PW_geneSTheatmapKp.png")

ggsave(height=9, width=8, file="../figs/FigureS12_PW_geneSTheatmapKp.pdf")
...
#PW_dat_variant_nkp$variant[PW_dat_variant_nkp$variant == 'aac6'] <- 'aac(6`)-Ib-cr'

PW_dat_variant_nkp$ST <- gsub("ST", "", PW_dat_variant_nkp$ST)

# ST vs variant, for STs with N>=100 (nkp)
PW_dat_nkp <- PW_dat_variant_nkp %>%
  filter(ST %in% ST_freq$ST[ST_freq$species!="Klebsiella pneumoniae" & ST_freq$N>10]) %>%
  group_by(ST, variant) %>% count() %>%
  left_join(ST_freq, by="ST") %>%
  mutate(pc=n/N)
...
nkp_st_order <- c(ST_freq %>% filter(species=="Klebsiella variicola subsp. variicola") %>% filter(N>10) %>% pull(ST), ST_freq %>% filter(grepl("subsp. quasi", species)) %>% filter(N>10) %>% pull(ST), ST_freq %>% filter(grepl("subsp. simili", species)) %>% filter(N>10) %>% pull(ST))

spp_cols <- c("Klebsiella pneumoniae"="black","Klebsiella quasipneumoniae subsp. quasipneumoniae"="navy","Klebsiella quasipneumoniae subsp. similipneumoniae"="maroon","Klebsiella variicola subsp. variicola"="orange",
            "Klebsiella quasivariicola"="khaki3","Klebsiella africana"="navajowhite4")
...
# Figure S13 - PW determinants vs ST (non-Kp)
...
ggsave(height=9, width=7, file="../figs/FigS13_PW_geneSTheatmapNonKp.png")

ggsave(height=9, width=7, file="../figs/FigS13_PW_geneSTheatmapNonKp.pdf")
...
MIC_profile_colours <- c("0.25 mg/L [0.25-0.25]" = "#add8e6", "0.25 mg/L [0.25-0.5]"="#72bcd4", "0.5 mg/L [0.5-1]"="#78638a", "1 mg/L [1-2]"="#fadbd8", "2 mg/L [1-2]"="#f1948a", "2 mg/L [2-4]"="#C64343", "4 mg/L [4-4]"="#4A0404")


PW$Ciprofloxacin_MIC_prediction <- factor(PW$Ciprofloxacin_MIC_prediction, levels=c("0.25 mg/L [0.25-0.25]", "0.25 mg/L [0.25-0.5]", "0.5 mg/L [0.5-1]", "1 mg/L [1-2]", "2 mg/L [1-2]", "2 mg/L [2-4]","4 mg/L [4-4]"))


MIC_year_pathogenwatch_plot <- PW  %>% 
  filter(species=="Klebsiella pneumoniae")  %>% 
   group_by(year, Ciprofloxacin_MIC_prediction) %>% count() %>%
  ggplot(aes(fill=as.factor(Ciprofloxacin_MIC_prediction), y=n, x=year)) + 
...
ggsave(height=10, width=8, file="../figs/FigS14_PW_profile_year.pdf")
ggsave(height=10, width=8, file="../figs/FigS14_PW_profile_year.png")
...
PW %>% group_by(Country) %>% count()%>% filter(n>100)

PW %>% filter(species=="Klebsiella pneumoniae") %>% group_by(Country) %>% count()%>% filter(n>100)
...
ggsave(height=3, width=8, file="../figs/Fig8_PW_consumption.pdf")
ggsave(height=3, width=8, file="../figs/Fig8_PW_consumption.png")

```

# Not used - country vs. genotype profile
