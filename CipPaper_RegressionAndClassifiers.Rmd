...
model_R_indiv_logistf <- logistf(resistant ~ ., data=R_vs_dets)
...
plot_summs(model_R_indiv, inner_ci_level = .9, omit.coefs=c("(Intercept)", "qnrB17"))
plot_summs(model_R_indiv_aac6Interaction, inner_ci_level = .9, omit.coefs=c("(Intercept)", "qnrB17"))

summ(model_R_indiv, exp = TRUE, omit.coefs=c("(Intercept)", "qnrB17"))
```
...
metrics <- function(pred,truth) {
  xtabs <- table(pred,truth)
  return(list(cat=(xtabs[1,1]+xtabs[2,2])/sum(xtabs),ppv=xtabs[2,2]/sum(xtabs[2,]),npv=xtabs[1,1]/sum(xtabs[1,]),me=xtabs[2,1]/sum(xtabs[,1]),vme=xtabs[1,2]/sum(xtabs[,2]),n=sum(xtabs), summary=c("cat"=(xtabs[1,1]+xtabs[2,2])/sum(xtabs),"ppv"=xtabs[2,2]/sum(xtabs[2,]),"npv"=xtabs[1,1]/sum(xtabs[1,]),"me"=xtabs[2,1]/sum(xtabs[,2]),"vme"=xtabs[1,2]/sum(xtabs[,1]), "n"=sum(xtabs))))
}
